from keras.layers import Conv2D, Input, BatchNormalization, LeakyReLU, ZeroPadding2D, UpSampling2D, Lambda, GlobalAveragePooling3D, Dense, Softmax
...
def _conv_block(inp, convs, do_skip=True):
    ...
    return x #add([skip_connection, x]) if do_skip else x        

def Convolution(inp, filter, kernel, stride, layer_index, bnorm=True, leaky=True):
    x = inp     
    if stride > 1: x = ZeroPadding2D(((1,0),(1,0)))(x)
    x = Conv2D(filter, 
                kernel, 
                strides=stride, 
                padding='valid' if stride > 1 else 'same', # unlike tensorflow darknet prefer left and top paddings
                name='conv_' + str(layer_index), 
                use_bias=False if bnorm else True)(x)
    if bnorm: x = BatchNormalization(epsilon=0.001, name='bnorm_' + str(layer_index))(x)
...
def create_yolov3_model(nb_class, anchors, max_box_per_image,  max_grid, batch_size, warmup_batches, ignore_thresh, grid_scales, obj_scale, noobj_scale, xywh_scale, class_scale):
    ...
    #Die hier raus
    x = Convolution(inp=x, layer_index=12, filter=256, kernel=3, stride=2)
    ...
    for i in range(8):
        ...
        ###Die hier auch
        x = Convolution(inp=x, layer_index=39+i*3, filter=512, kernel=3, stride=1)
